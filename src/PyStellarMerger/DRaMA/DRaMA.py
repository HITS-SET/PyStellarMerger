...
def read_input(infile):
    """
    Reads input parameters from infile and checks their validity.
    """
    # Set default values
    defaults = {'primary_star': 'primary.data', 
            'secondary_star': 'secondary.data', 
            'disrupted_star': 'secondary',
            'flavour': 'shift',
            'scale factor': 4/3,
            'radius mask': (1,1),
            'outside mass': 'attached',
            'smoothing denominator': 200 }
...
def survivor_pressure_scale_height(
    mass_1, logR_1, pressure_1, density_1,
    floor_g_cgs: float = 1e-30,
    floor_density_cgs: float = 1e-99,
) -> dict[str, np.ndarray]:
    """
    Computes the pressure scale height profile of the survivor star, which is needed for the DRaMA mixing schemes. 
    Uses H_P = P / (rho * g) with g = G * M_enc / r^2, where M_enc is the enclosed mass at radius r. 
    """
    radius_1 = 10.0 ** logR_1
    radius_1_cm = radius_1 * R_SUN_CM
    enclosed_mass_g = mass_1 * M_SUN_G

    g_cgs = G_CGS * enclosed_mass_g / np.maximum(radius_1_cm, 1e-99) ** 2
    g_cgs = np.maximum(g_cgs, floor_g_cgs)
    ...
    first_good = int(np.flatnonzero(finite)[0])
    hp_rsun = np.where(np.isfinite(hp_rsun), hp_rsun, hp_rsun[first_good])
    hp_rsun = np.maximum(hp_rsun, 0.0)

    return dict(mass_1=mass_1, radius_1=radius_1, Hp_rsun_1=hp_rsun)

def tidal_disruption_radius_innerX(mass_1, logR_1, mass_2, logR_2, X):
    """
    ...
    def tidal_expr(d, R_comp):
        return R_comp**2 * (2.0 * d * R_comp - R_comp**2) / (d**2 * (d - R_comp)**2)

    def solve_scalar(R_comp, M_host, M_comp):
        q = M_comp / M_host
        f = lambda d: tidal_expr(d, R_comp) - q
        try:
            return brentq(f, R_comp * 1.001, R_comp * 1e6)
        except ValueError:
            return None

    def residual_array(r_array, mass_host, R_comp, M_comp):
        q = M_comp / mass_host
        valid = r_array > R_comp
        res = np.full_like(r_array, np.inf, dtype=float)
        d_v = r_array[valid]
        q_v = q[valid]
        res[valid] = np.abs(tidal_expr(d_v, R_comp) - q_v)
        return res

    r_tid_2_full = solve_scalar(R2_X, M1_final, X * M2_final)
    if r_tid_2_full is not None and r_tid_2_full >= R1_final:
        r_tid_2 = r_tid_2_full; M_tid_2 = M1_final
    else:
        res_2 = residual_array(r_1, mass_1, R2_X, X * M2_final)
        idx2 = np.argmin(res_2)
        r_tid_2 = r_1[idx2]; M_tid_2 = mass_1[idx2]

    return dict(r_tid_2=r_tid_2, M_tid_2=M_tid_2)

def ram_pressure_radius_innerX(mass_1, radius_1, density_1, mass_2, pressure_2, X, sp_value: float = 0.88):
    """
    Computes the ram pressure stripping radius for the mass shell of star 2 (disrupted star) closest at mass coordinate X. 
    """
    G_cgs = 6.67430e-8
    M_sun_g = 1.989e33
    R_sun_cm = 6.957e10
    ...
    radius_1_cm = radius_1 * R_sun_cm
    M_enc_1 = mass_1 * M_sun_g

    M2_tot = float(mass_2[0])
    target_mass = X * M2_tot

    idx_shell = int(np.argmin(np.abs(mass_2 - target_mass)))
    m_shell = float(mass_2[idx_shell])
    p_shell = float(pressure_2[idx_shell])

    M_rest_g = m_shell * M_sun_g
    v_orb = np.sqrt(G_cgs * (M_enc_1 + M_rest_g) / np.maximum(radius_1_cm, 1e-99))
    p_ram = sp_value * density_1 * v_orb**2

    crossing = np.where(p_ram >= p_shell)[0]
    if len(crossing) == 0:
        return None

    idx_cross = int(crossing[0])
    r_ram = float(radius_1[idx_cross])
    M_ram = float(mass_1[idx_cross])

    return dict(r_ram=r_ram, M_ram=M_ram)
